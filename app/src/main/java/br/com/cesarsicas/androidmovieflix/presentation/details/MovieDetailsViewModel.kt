package br.com.cesarsicas.androidmovieflix.presentation.details

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import br.com.cesarsicas.androidmovieflix.domain.model.TitleDetailsModel
import br.com.cesarsicas.androidmovieflix.domain.usecase.GetTitleDetailsUseCase
import br.com.cesarsicas.androidmovieflix.presentation.common.UiState
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.launch
import javax.inject.Inject

@HiltViewModel
class MovieDetailsViewModel @Inject constructor(
    private val getTitleDetailsUseCase: GetTitleDetailsUseCase,
) : ViewModel() {

    private val _state = MutableStateFlow<UiState<TitleDetailsModel>>(UiState.Loading)
    val state: StateFlow<UiState<TitleDetailsModel>> = _state

    fun loadDetails(externalId: Int) {
        viewModelScope.launch {
            _state.value = UiState.Loading
            _state.value = try {
                UiState.Success(getTitleDetailsUseCase(externalId))
            } catch (e: Exception) {
                UiState.Error(e.message ?: "Failed to load details")
            }
        }
    }
}
